...
$courses = $pdo->query("SELECT c.*, t.name AS teacher_name FROM courses c LEFT JOIN teachers t ON c.teacher_id=t.id")->fetchAll();
...
<div class="content">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addModal">➕ إضافة مادة جديدة</button>
    </div>

    <div class="card">
        <table class="table table-hover align-middle">
            <thead>
                <tr><th>اسم المادة</th><th>الوصف</th><th>المعلم المسؤول</th></tr>
            </thead>
            <tbody>
    ...
    <div class="modal-dialog"><form method="POST" class="modal-content">
        ...
        <div class="modal-body">
            ...
            <select name="teacher_id" class="form-control mb-2">
                ...
                <?php foreach($teachers as $teacher): ?><option value="<?= $teacher['id'] ?>"><?= $teacher['name'] ?></option><?php endforeach; ?>
            </select>
        </div>
        <div class="modal-footer"><button type="submit" name="add_course" class="btn btn-primary">حفظ المادة</button></div>
    </form></div>
</div>
