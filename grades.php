...
// جلب البيانات
$grades = $pdo->query("SELECT g.*, s.name AS student_name, c.name AS course_name FROM grades g JOIN students s ON g.student_id=s.id JOIN courses c ON g.course_id=c.id ORDER BY g.exam_date DESC")->fetchAll();
...
<div class="content">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2>سجل الدرجات</h2>
        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addModal">➕ إضافة درجة جديدة</button>
    </div>

    <div class="card">
        <table class="table table-hover align-middle">
            <thead>
                <tr><th>الطالب</th><th>المادة</th><th>الدرجة</th><th>التاريخ</th></tr>
            </thead>
            <tbody>
    ...
    <div class="modal-dialog"><form method="POST" class="modal-content">
        ...
        <div class="modal-body">
            <select name="student_id" class="form-control mb-2" required>
                ...
                <?php foreach($students as $s): ?><option value="<?= $s['id'] ?>"><?= $s['name'] ?></option><?php endforeach; ?>
            </select>
            <select name="course_id" class="form-control mb-2" required>
                <option value="">اختر المادة</option>
                <?php foreach($courses as $c): ?><option value="<?= $c['id'] ?>"><?= $c['name'] ?></option><?php endforeach; ?>
            </select>
        ...
        <div class="modal-footer"><button type="submit" name="add_grade" class="btn btn-primary">حفظ الدرجة</button></div>
    </form></div>
</div>
